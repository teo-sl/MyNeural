...
public class Layer implements Serializable {
    private static double lr=0.1,gamma=0.9;
    private double[][] weights,delta_w, v_t_weights;
    private double[] biases,delta_b,v_t_biases;
    ...
    public Layer(int neuronNumber,int inputDim,ActivationFunction activationFun) {
        ...
        this.v_t_biases=new double[neuronNumber];
        Arrays.fill(v_t_biases,0);

        this.weights=new double[neuronNumber][inputDim];
        this.delta_w=new double[neuronNumber][inputDim];
        this.v_t_weights =new double[neuronNumber][inputDim];
        Matrixes.reset(v_t_weights);
    ...
    public void updateAll(int n) {
        for(int i=0;i<neuronNumber;++i) {
            v_t_biases[i]=gamma*v_t_biases[i]+delta_b[i]/n;
            biases[i]-=v_t_biases[i];

        }

        for(int i=0;i<neuronNumber;++i)
            for(int j=0;j<inputDim;++j) {
                v_t_weights[i][j]=gamma*v_t_weights[i][j]+delta_w[i][j]/n;
                weights[i][j]-=v_t_weights[i][j];
